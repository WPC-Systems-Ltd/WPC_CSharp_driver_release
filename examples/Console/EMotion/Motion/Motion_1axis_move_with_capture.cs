...
class EMotion_1axis_move_with_capture
{
    ...
    { 
        ...
        {   
            ...
            status = dev.Motion_cfgCapture(port, Constant.MOTION_AXIS_1, Constant.MOTION_CAPTURE_RISING_EDGE_TYPE, Constant.MOTION_CAPTURE_LOGICAL_POSITION_MODE);
            Console.WriteLine($"Motion_cfgCapture status: {status}");

            status = dev.Motion_setCaptureEnable(port, Constant.MOTION_AXIS_1, Constant.MOTION_ENABLE_TRUE);
            Console.WriteLine($"Motion_setCaptureEnable status: {status}");
            ...
            while (move_status == 0)
            {
                ...
                int capture_points = dev.Motion_readCapturePoint(port, Constant.MOTION_AXIS_1);
